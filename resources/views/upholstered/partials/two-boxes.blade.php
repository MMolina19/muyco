        ...
        <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative text-dark">
            <div class="col p-4 d-flex flex-column position-static bg-light">
                <strong class="d-inline-block mb-2 text-success">
                    {{__('upholstered.singular-title')}}
                </strong>

                <h3 class="mb-0">{{$upholstered->name}}</h3>
                <div class="mb-1 text-muted">{{$upholstered->active}}</div>
                <div class="mb-1 text-muted">{{$upholstered->created_at}}</div>
                <div class="mb-1 text-muted">{{$upholstered->updated_at}}</div>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img class="bd-placeholder-img" width="200" height="250"
                src="{{asset('images/thumbnails/200x250/thumbnail_200_x_250_silla_mod20_01.jpg')}}"
                role="img" placeholder="{{trans('upholstered.title')}}" focusable="false" />
            </div>
        </div>
